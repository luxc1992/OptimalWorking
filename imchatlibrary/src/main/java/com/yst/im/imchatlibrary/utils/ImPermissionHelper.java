...
public class ImPermissionHelper {
    ...
    private static final int REQUEST_PERMISSION_CODE = NUM_1000;
    ...
    public ImPermissionHelper(@NonNull Object object) {
        ...
        switch (requestCode) {
            case REQUEST_PERMISSION_CODE:
                ...
                if (allGranted && mListener != null) {
                    mListener.doAfterGrand(mPermissionList);
                } else if (!allGranted && mListener != null) {
                    mListener.doAfterDenied(mPermissionList);
                }
                break;
            default:
                break;
    ...
    private void checkCallingObjectSuitability(@Nullable Object object) {
        ...
        boolean isParentForm = (isSupportFragment || isActivity || (isAppFragment && isNeedRequest()));
        if (!isParentForm) {
            if (isAppFragment) {
