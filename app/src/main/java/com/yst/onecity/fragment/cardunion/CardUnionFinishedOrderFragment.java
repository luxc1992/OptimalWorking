 ...
 * @date 2018/5/17
 */
public class CardUnionFinishedOrderFragment extends LazyBaseFragment implements OnRefreshListener, OnLoadmoreListener {
...
//        requestData(page);
    ...
    private void requestData(final int page) {
        RequestApi.getCardUnionOrderList(App.manager.getPhoneNum(), App.manager.getUuid(), App.manager.getId()+"", "1", page, new AbstractNetWorkCallback<CardUnionOrderBean>() {
            @Override
            public void onAfter() {
                super.onAfter();
                loadFinish();
            ...
            public void onSuccess(CardUnionOrderBean cardUnionOrderBean) {
                if (cardUnionOrderBean.getCode() == NO1){
                    boolean b = page == NO1 && (cardUnionOrderBean.getContent().getCardOrderVOList() == null || cardUnionOrderBean.getContent().getCardOrderVOList().size() == 0);
                    if (b) {
                    ...
                    ToastUtils.show(cardUnionOrderBean.getMsg());
                }
