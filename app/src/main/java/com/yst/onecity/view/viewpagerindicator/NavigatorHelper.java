...
import static com.yst.onecity.Constant.NO0F;
...
public class NavigatorHelper {
    ...
    private void dispatchOnLeave(int index, float leavePercent, boolean leftToRight, boolean force) {
        boolean isLast = index == mLastIndex;
        boolean isState = mScrollState == ScrollState.SCROLL_STATE_DRAGGING;
        boolean isFrame = (index == mCurrentIndex - 1 || index == mCurrentIndex + 1) && mLeavedPercents.get(index, 0.0f) != 1.0f;
        if (mSkimOver || isLast || isState || isFrame || force) {
            if (mNavigatorScrollListener != null) {
