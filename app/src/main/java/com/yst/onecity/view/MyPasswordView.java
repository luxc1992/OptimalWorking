 ...
 * @date 2018/02/09
 */
public class MyPasswordView extends android.support.v7.widget.AppCompatEditText {

    private static final int DEFAULT_CONT_MARGIN = 5;
    private static final int DEFAULT_SPLIT_LINE_WIDTH = 4;
    ...
    public MyPasswordView(Context context, AttributeSet attrs) {
        ...
        splitLineWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, splitLineWidth, dm);
        ...
        splitLineWidth = a.getInt(R.styleable.PasswordInputView_pivSplitLineWidth,splitLineWidth);
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 内容区
        RectF rectIn = new RectF(rect.left + DEFAULT_CONT_MARGIN, rect.top + DEFAULT_CONT_MARGIN,
                rect.right - DEFAULT_CONT_MARGIN, rect.bottom - DEFAULT_CONT_MARGIN);
        borderPaint.setColor(ContextCompat.getColor(getContext(),R.color.white));
        canvas.drawRoundRect(rectIn, borderRadius, borderRadius, borderPaint);

        // 分割线
        borderPaint.setColor(borderColor);
        borderPaint.setStrokeWidth(splitLineWidth);
