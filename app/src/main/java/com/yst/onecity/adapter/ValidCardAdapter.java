 ...
 * @date 2018/5/18
 */
public class ValidCardAdapter extends BaseAdapter {
    private List<MyCardBagListBean.ContentBean.TrueMapBean.TrueArrayBean> list;
    private Context context;

    public ValidCardAdapter(List<MyCardBagListBean.ContentBean.TrueMapBean.TrueArrayBean> list, Context context) {
        this.list = list;
        this.context = context;
    }

    public void setData(List<MyCardBagListBean.ContentBean.TrueMapBean.TrueArrayBean> list) {
        if (list != null) {
            this.list = list;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ValidCardViewHolder vh;
        if (convertView == null) {
            convertView = LayoutInflater.from(context).inflate(R.layout.item_validcard, null);
            vh = new ValidCardViewHolder(convertView);
            convertView.setTag(vh);
        } else {
            vh = (ValidCardViewHolder) convertView.getTag();
        }
        ...
        if (list.get(position).getType() == NO0 || list.get(position).getType() == NO1) {
            vh.layout.setBackgroundResource(R.drawable.shape_cardbaglist_bg);
            vh.tvCardExplain.setText("注：仅限本人使用，每天限用一位商家");
        }
        //次数卡
        if (list.get(position).getType() == NO2 || list.get(position).getType() == NO3) {
            vh.layout.setBackgroundResource(R.drawable.shape_cardbaglist_bg2);
