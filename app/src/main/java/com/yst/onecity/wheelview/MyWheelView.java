...
import com.yst.onecity.wheelview.adapter.MyWheelViewAdapter;
...
import com.yst.onecity.wheelview.listener.MyOnWheelScrollListener;
...
public class MyWheelView extends View {
	...
	private int[] lowerSize= new int[] { 12,
			14, 16 };
	private int[] lowerColors = new int[] {0xFFFFFFFF, 0x00FFFFFF, 0x00FFFFFF };
	...
	public void setShadowColor(int start, int middle, int end) {
		lowerColors = new int[] {start, middle, end};
	}

	public void setShadowFontSize(int start, int middle, int end) {
		lowerSize = new int[] {start, middle, end};
	}
	...
	public void setWheelForeground(int resource) {
		...
		ContextCompat.getDrawable(getContext(),wheelForeground);
	}
	...
	private void initResourcesIfNecessary() {
		...
		if (topShadow == null) {
			topShadow = new GradientDrawable(Orientation.TOP_BOTTOM, lowerColors);

		}

		if (bottomShadow == null) {
			bottomShadow = new GradientDrawable(Orientation.BOTTOM_TOP, lowerColors);
		}
	...
	private void doScroll(int delta) {
		...
		if (Math.abs(fixPos) <= itemHeight / str) {
			fixPos = 0;
		}
