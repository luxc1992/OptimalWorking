...
import static com.yst.im.imsdk.MessageConstant.NUM_31;
import static com.yst.im.imsdk.utils.BaseUtils.getSDFreeSize;
import static com.yst.im.imsdk.utils.BaseUtils.hasSdcard;
...
public class LocalLog {
    static public String logPath = "/sdcard/ImLocalLog";
    static private boolean androidLogOn = false;
    ...
    public static void e(String tag, String msg) {
        ...
        if (localLogOn && hasSdcard() && getSDFreeSize() > NUM_1) {
    ...
    private static void printToFile(int priority, String tag,
        ...
        @SuppressLint("DefaultLocale") String timeString = String.format("%d-%02d-%02d %02d:%02d:%02d",
        ...
        switch (priority) {
            case INFO:
                logFileName = "%s/" + fileName + "_Info%d%02d%02d%02d%02d.%s";
                break;
            case ERROR:
                logFileName = "%s/" + fileName + "_Error%d%02d%02d%02d%02d.%s";
                break;
            case FAIL:
                logFileName = "%s/" + fileName + "_Fail%d%02d%%02d%02d.%s";
                break;
            case SUCCESS:
                logFileName = "%s/" + fileName + "_Success%d%02d%02d%02d%02d.%s";
                break;
            default:
                logFileName = "%s/" + fileName + "%d%02d%02d.%s";
        }
        logFileName = String.format(logFileName, logpath, year, month, day, hour, minute, fileType);
